...
반면 그 아래 백엔드 로직은 대부분 진짜다: 11,256줄의 비테스트 Go 코드, 26개 패키지 테스트 전부 통과, 스텁·`panic`·빈 함수 0개.
...
| `go vet ./...` | ❌ 동일 원인 실패 |
| `go test ./...` | ⚠️ 26개 패키지 통과, 2개 `[setup failed]` (동일 embed 원인). **실패한 테스트 단언은 0건** |
...
- 컴파일되는 **26개 패키지 테스트 전부 통과**, 실패 단언 0건. 탐지기는 실제 TLS 서버를 띄워 통합 검증한다.
