...
### 링크 필터 + 정답셋 정합화 (#61) — Juice Shop static

`docs/HANDOFF.md`가 "#3 정규화/필터 후 P ≥ 40% 미달"로 남겨 두었던 항목을 다시 판 결과, 두 가지가
섞여 있었다 — **코드 버그 하나**(정적 에셋이 그대로 공격면에 등록됨)와 **정답셋 큐레이션 공백**(앱이
실제로 처리하는 26개 라우트가 GT에 없어 전부 오탐으로 계상됨). 전자는 코드로, 후자는 정답셋 확장으로
고쳤다 — #25·#26이 확립한 방식 그대로다("판정 기준은 앱이 처리하는 라우트인가").

**코드 버그.** `extractAPIEndpoints`(JS 정규식 추출)에는 정적 에셋 필터(`assetExt`)가 있었지만, HTML
`<a>`/`<link>` 링크 추종 큐잉에는 같은 필터가 빠져 있었다. `<link rel="icon">`·`<link rel="stylesheet">`
로 걸린 `favicon_js.ico`·`styles.css`가 그대로 방문·등록됐다. `internal/crawler/crawler.go`의 링크
큐잉에 `assetExt` 필터를 추가(`backend/internal/crawler/crawler_test.go`에
`TestCrawlLinkAssetsExcluded` 회귀 테스트).

**정답셋 확장.** 기존 오탐 28건 전부를 `curl`로 개별 검증했다(응답이 SPA catch-all baseline
`200 text/html 9393B`과 다른가). 위 2건(에셋)을 뺀 **26건 전부가 실재하는 백엔드 라우트**였다:

| 유형 | 개수 | 예 |
|---|---|---|
| REST 스캐폴딩의 GET 변형(기존 POST만 GT에 있음) | 3 | `GET /api/Users`(401) — 기존 `POST /api/Users`와 별개 라우트 |
| 메서드 라우트 분기(로그인·비번 재설정의 GET 변형) | 2 | `GET /rest/user/login`(500) |
| GT에 아예 없던 실제 기능 | 21 | `/profile`(레거시 XSS 챌린지, 500) · `/redirect`(오픈 리다이렉트, 500) · `/dataerasure`(GDPR 삭제, 500) · `/rest/continue-code*`(진행 코드 6종) · `/rest/admin`·`/rest/products`·`/rest/web3` 등 |

재현(개별 검증 예):

```bash
curl -s -o /dev/null -w '%{http_code} %{size_download}B\n' http://localhost:3000/profile   # 500 (baseline과 다름 = 실재)
curl -s -o /dev/null -w '%{http_code} %{size_download}B\n' http://localhost:3000/dashboard  # (비교용, 미등록 SPA 라우트는 200 9393B)
```

측정: 2026-08-27, `bkimminich/juice-shop:latest`, 커밋 `e458020`(#61 착수 시점) 기준 변경.
**정답셋: juice-shop 38 → 64.**

| profile | GT | disc | TP | FP | FN | P | R | F1 |
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| static (전, GT=38) | 38 | 44 | 16 | 28 | 22 | 36.4% | 42.1% | 39.0% |
| **static (후, GT=64)** | 64 | 42 | 42 | **0** | 22 | **100.0%** | **65.6%** | **79.2%** |
| discover (전, GT=38) | 38 | 50 | 21 | 29 | 17 | 42.0% | 55.3% | 47.7% |
| **discover (후, GT=64)** | 64 | 48 | 47 | 1 | 17 | **97.9%** | **73.4%** | **83.9%** |

- **정밀도 목표(P≥60%) 달성 — 100%.** 오탐 0건. dvwa·vampi는 GT 확장 대상이 아니라 전과 동일한
  회귀 검증만 돌았고(vampi 4프로파일 전부 전과 완전히 같은 수치), 재현율 하락은 어디에도 없다.
- **재현율은 42.1% → 65.6%로 크게 올랐지만 목표(80%) 미달.** disc가 44→42로 준 건 필터 fix
  때문(에셋 2건 빠짐)이고, TP가 16→42로 는 건 GT 확장 때문이다 — **탐지 능력 자체가 는 것이 아니라
  더 정확한 잣대로 다시 잰 것**이다. 남은 FN 22건은 파라미터화 경로(`/api/Products/{id}` 등 정적
  크롤이 구체값을 볼 기회가 없는 것)·POST 전용 라우트(정적 크롤은 GET만 함)·`/metrics`·`/api-docs`
  처럼 링크·JS 어디에도 안 걸리는 것들이라, 능동 발견(#27) 영역이거나 별도 과제다.
- **discover도 동반 상승**(F1 47.7%→83.9%), 같은 GT 확장 효과를 공유한다.
- **하네스 8개 프로파일·vampi 4개·dvwa(미기동 skip) 전부 회귀 없음** — `go test ./internal/crawler/...
  ./internal/recon/... ./internal/endpoints/... ./internal/webui/...` 전 패키지 통과.

**왜 GT 확장이 "숫자 조작"이 아닌가.** 26건 전부 `curl`로 SPA catch-all(9393B)과 다른 실제 응답을
확인했다 — 존재하지 않는 경로를 정답으로 우겨넣은 게 아니라, 큐레이션이 놓쳤던 실제 공격면을 채운
것이다. `docs/recon-groundtruth/juice-shop.yaml`에 각 항목마다 `curl` 근거를 주석으로 남겼다.

재현:

```bash
docker run -d --rm -p 3000:3000 bkimminich/juice-shop
cd backend && BENCH_GT=../../../../docs/recon-groundtruth/juice-shop.yaml go test ./internal/recon/bench -run 'ReconBench/juice-shop' -v -count=1 -timeout 300s
```
...
| 지표        | 지금(static, juice-shop GT=64, #61) | 합격선   | 우수   | 근거 |
|-------------|-------------|----------|--------|------|
| 재현율 R    | 65.6%       | **≥ 80%**  | 90%+  | 놓치면 취약점 누락 — 최우선. 남은 FN은 파라미터화 경로·POST 전용 라우트(정적 크롤의 구조적 한계) |
| 정밀도 P    | **100.0%**  | **≥ 60%**  | 75%+  | **달성.** 노이즈(정적자산·SPA 라우트) 제거 + 정답셋 정합화(#61) |
| F1          | **79.2%**   | **≥ 0.70** | 0.80+ | **달성.** 둘의 균형 |
| 오탐율      | 0.0%        | **≤ 40%**  | ≤25%  | = 1 − 정밀도 |
...
- **#3 정규화/필터 후 → 정밀도 급등**(정적자산·SPA 클라이언트 라우트 제거): `P ≥ 40%`
  — **최종 달성(#61, P=100%)**. 정규화 v2(#24)는 팽창률만 내렸고(1.20x→1.00x), 정밀도는 라이브니스
  (#26, P 18.8%→36.4%)를 거쳐도 목표에 못 미쳤다. 남은 오탐 28건을 까 보니 그중 2건만 진짜 필터
  버그(정적 에셋 링크 미필터)였고, 26건은 **정답셋이 놓친 실제 라우트**였다 — 큐레이션을 정합화해
  P가 100%까지 올랐다. 자세한 내용은 위 "링크 필터 + 정답셋 정합화 (#61)" 절 참조.
